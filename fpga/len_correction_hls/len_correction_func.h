...
namespace vr_prototype
{
	class Len_corrector
	{
		...
		void operator()(hls::stream<Pixel> &dout, hls::stream<Agg_in_srgb> &din) {
			...
			#pragma HLS STREAM variable=bilinear_info_stream depth=512 // too long?
			#pragma HLS STREAM variable=memory_write_stream depth=32
			#pragma HLS STREAM variable=memory_read_stream depth=32
			#pragma HLS STREAM variable=read_data_stream depth=32
		...
		void memory_manager(hls::stream<FourPixel> &read_data_stream, hls::stream<Memory_access> &memory_write_stream, hls::stream<Memory_access> &memory_read_stream) {
			...
			#pragma HLS AGGREGATE compact=bit variable=pixel_buffer_1 // need to aggregate to prevent 3x resource usage 24xd  instead 8xd 8xd 8xd (3x memory usage)
			#pragma HLS AGGREGATE compact=bit variable=pixel_buffer_2
			#pragma HLS AGGREGATE compact=bit variable=pixel_buffer_3
			#pragma HLS AGGREGATE compact=bit variable=pixel_buffer_4
			#pragma HLS bind_storage variable=pixel_buffer_1 type=ram_1p impl=uram
			#pragma HLS bind_storage variable=pixel_buffer_2 type=ram_1p impl=uram
			#pragma HLS bind_storage variable=pixel_buffer_3 type=ram_1p impl=uram
			#pragma HLS bind_storage variable=pixel_buffer_4 type=ram_1p impl=uram
		...
		void write_to_buffer(Memory_access &in, Pixel pixel_buffer_1[94][480], Pixel pixel_buffer_2[94][480], Pixel pixel_buffer_3[94][480], Pixel pixel_buffer_4[94][480]) {
			// std::cout addresses
		...
		void read_from_buffer(hls::stream<FourPixel> &read_data_stream, Memory_access &in, Pixel pixel_buffer_1[94][480], Pixel pixel_buffer_2[94][480], Pixel pixel_buffer_3[94][480], Pixel pixel_buffer_4[94][480]){
			FourPixel out;
			// std::cout << "address1: " << in.address1[0] << " " << in.address1[1] << " " << in.address1[2] << " " << in.address1[3] << std::endl;
			// std::cout << "address2: " << in.address2[0] << " " << in.address2[1] << " " << in.address2[2] << " " << in.address2[3] << std::endl;
			...
			for (int i = 0; i < 1080; i++)
			{
				for (int j=0; j < 961; j++)
				{
					...
					if (j == 960) {
						...
						if(j == 0) {
							auto tmp = memory_info_stream.read();
						}
		...
		void send_read_query(hls::stream<Memory_access> &memory_read_stream, hls::stream<Bilinear_info> &bilinear_info_stream, const float &cor_x,  const float &cor_y, const bool &valid) {
			#pragma HLS INLINE // need this Inline or II=1 will fail
