...
struct Agg_in_srgb { // here input is rgb format , not bgr
	ap_uint<8> rgb[16][3];
};
    ...
    )
    {
        ...
        for(int i = 0; i < total_num; i++)
        {
            ...
            if (i < load_num)
            {
                ...
                // add a tile to the 4-row buffer
                for(int j = 0; j < tile_size * tile_size; j++)
                {
                    int row_idx = j / tile_size;
                    int col_idx = j % tile_size + i * tile_size; // shift + base address of the tile
