...
class BaseColorModel(Module):
    ...
    def eval(self, x):
        ...
        if isinstance(x, cp.ndarray):
            x = torch_from_dlpack(x.toDlpack())
        return self.model.forward(x)
    

    def eval_cupy(self, x):
        x = torch_from_dlpack(x.toDlpack())
        return self.model.forward(x)
    ...
    def compute_ellipses_gpu(self, img, ecc_map):
        ...
        inp = cp.concatenate((contrasts, ecc_map), axis=-1)
        with torch.no_grad():
            c_abc = self.eval_cupy(inp).detach()
        
        c_abc = cp.fromDlpack(torch_to_dlpack(c_abc))
...
class Network(nn.Module):
    def __init__(self, layer_widths, layer_centres, basis_func):
        ...
        self.lm_max = 0.3025
        self.s_max = 0.00655
        w = torch.tensor([1 / self.lm_max, 1 / self.s_max, 1, 1/35], dtype=torch.float32).cuda()
        b = torch.tensor([0, 0, 0, 0], dtype=torch.float32).cuda()
        self.normalize = lambda x: w * x + b
        self.denormalize = lambda y: (y - b) * w

        self.final_vec = torch.tensor([self.lm_max, self.s_max, self.s_max]).cuda()
