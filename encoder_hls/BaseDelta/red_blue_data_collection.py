"""
This is a copy of red_blue_optimization.py code with data collection for a linear classifier. Data collection includes the original rgb
values of the pixels in a given tile, the eccentricity used for each pixel in the tile, and whether it was determined that that
tile should be optimized in the red or blue direction. This data is saved to a csv file that can be used to train a 
linear classifier in red_blue_vlassifier.py
Work in Progress, these variables were not determined to be capable of training an accurate classifer to choose between 
red and blue optimization
"""
...
# higher C value means higher compression rate and lower percpetual quality
ECC = 25        # could be 10, 25, or 35  # used for constant eccentricity, not foveated 
C_VALUE = 1e-3  # adjustable, original: 1e-5

# vectors spanning ellipsoids from min values to max values, traversed in process of optimization
...
# for data collection
df = pd.DataFrame([], columns=['r0', 'r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8', 'r9', 'r10', 'r11', 'r12', 'r13', 'r14', 'r15',
                               'g0', 'g1', 'g2', 'g3', 'g4', 'g5', 'g6', 'g7', 'g8', 'g9', 'g10', 'g11', 'g12', 'g13', 'g14', 'g15',
                               'b0', 'b1', 'b2', 'b3', 'b4', 'b5', 'b6', 'b7', 'b8', 'b9', 'b10', 'b11', 'b12', 'b13', 'b14', 'b15',
                               'ecc0', 'ecc1', 'ecc2', 'ecc3', 'ecc4', 'ecc5', 'ecc6', 'ecc7', 'ecc8', 'ecc9', 'ecc10', 'ecc11', 'ecc12', 'ecc13', 'ecc14', 'ecc15',
                               'color'])
df.to_csv("./red_blue_ecc_data.csv", mode='w', index=False)
...
def optimize_tile(tile, ecc_tile, out):
    ...
    data = np.zeros((1,65)).astype(int)

    tile = tile.astype("int16")
    tile = tile.reshape(16,3)

    # data collection
    data[0,0:48] = tile.T.flatten()
    data[0,48:64] = ecc_tile.T.flatten()
    ...
    def generate_ellipsoids():
        ...
        return rgb_centers, dkl_centers, As
    ...
    rgb_centers, dkl_centers, As = generate_ellipsoids()
    ...
    if (r_bitlen_sum < b_bitlen_sum):
        if (np.any(red_opt_points < 0)):
            ...
            df.to_csv("./red_blue_ecc_data.csv", mode='a', index=False, header=False)
...
def color_conversion(image_dir, opt_dir, image_name, format, out):
    ...
    output_image = Image.fromarray(npNewImage)
